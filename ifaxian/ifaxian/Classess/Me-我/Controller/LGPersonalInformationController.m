...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    LGUserList *model = item.userInfos[indexPath.row];
    _userItem = model;
    if ([model.parameter isEqualToString:@"basic_user_avatar"]) {
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info{
    ...
    NSString *fileName = [NSString stringWithFormat:@"ifaxian/avatars/%@.jpg",name];
...
-(void)updataUserImage:(NSString *)url{
    ...
    [[LGNetWorkingManager manager] request:LGRequeTypePOST urlString:@"http://112.74.45.39/api/user/update_user_meta_vars" parameters:parameters completion:^(BOOL isSuccess, id responseObject) {
